...
class SummaryGenerator:
    ...
    async def generate_summary(self, chunks: List[TextChunk], paper_title: str = "") -> SummaryResult:
        ...
        # Create cache key from chunk contents
        content_hash = self._create_content_hash(chunks, paper_title)
        
        # Check cache first
        cached_result = self._get_cached_result(content_hash)
        if cached_result:
            logger.info(f"Using cached summary for content hash: {content_hash[:8]}")
    ...
    def _create_content_hash(self, chunks: List[TextChunk], paper_title: str) -> str:
        """Create a hash of the content for caching."""
        content_parts = [paper_title] + [chunk.content for chunk in chunks]
        combined_content = "|".join(content_parts)
        return hashlib.md5(combined_content.encode('utf-8')).hexdigest()
    
    def _get_cached_result(self, content_hash: str) -> Optional[SummaryResult]:
        """Get cached result if available."""
        # Simple in-memory cache (will be cleared on serverless function restart)
        if not hasattr(self, '_cache'):
            self._cache = {}
        return self._cache.get(content_hash)
    
    def _cache_result(self, content_hash: str, result: SummaryResult) -> None:
